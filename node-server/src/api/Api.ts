...
export class Api {
  ...
  public static async getAudioClassification(audioFile: UploadedFile): Promise<Dataset> {
    ...
    const classifiedResultsAttributesJson: string = await (fetch(`${process.env.MODEL_SERVICE}/audio/classify`, {
      method: 'POST',
      body: formData as any,
    }).then((res: any) => res.json()))

    return new Dataset(JSON.parse(classifiedResultsAttributesJson))
  }
}